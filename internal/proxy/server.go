...
func (ps *ProxyServer) HandleProxy(c *gin.Context) {
	...
	result := ps.retryHandler.ExecuteRequestWithRetry(c, channelHandler, originalGroup, group, finalBodyBytes, isStream, startTime, 0, model, excludedSubGroupIDs)
	...
	if result != nil && result.Response != nil {
		...
		if shouldInterceptModelList(c.Request.URL.Path, c.Request.Method) {
			...
			ps.requestLogger.LogRequest(c, originalGroup, group, apiKey, startTime, resp.StatusCode, nil, isStream, upstreamURL, channelHandler, finalBodyBytes, nil, models.RequestTypeFinal, model)
			return
		}
		...
		if isStream {
			tokenUsage = ps.handleStreamingResponse(c, resp, channelHandler, model, bodyBytes)
		} else {
			tokenUsage = ps.handleNormalResponse(c, resp, channelHandler, model, bodyBytes)
		}

		ps.requestLogger.LogRequest(c, originalGroup, group, apiKey, startTime, resp.StatusCode, nil, isStream, upstreamURL, channelHandler, finalBodyBytes, tokenUsage, models.RequestTypeFinal, model)
	}
}
