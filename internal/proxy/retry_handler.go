...
func (rh *RetryHandler) ExecuteRequestWithRetry(
	...
	if err != nil {
		...
		rh.requestLogger.LogRequest(c, originalGroup, group, nil, startTime, http.StatusServiceUnavailable, err, isStream, "", channelHandler, bodyBytes, nil, models.RequestTypeFinal, modelAlias)
		return nil
	}
	...
	if err != nil {
		...
		rh.requestLogger.LogRequest(c, originalGroup, group, apiKey, startTime, http.StatusInternalServerError, err, isStream, "", channelHandler, bodyBytes, nil, models.RequestTypeFinal, modelAlias)
		return nil
	}
	...
	if err != nil {
		...
		rh.requestLogger.LogRequest(c, originalGroup, group, apiKey, startTime, http.StatusBadRequest, err, isStream, upstreamURL, channelHandler, bodyBytes, nil, models.RequestTypeFinal, modelAlias)
		return nil
	}
	...
	if err != nil || (resp != nil && resp.StatusCode >= 400 && resp.StatusCode != http.StatusNotFound) {
		...
		if err != nil && app_errors.IsIgnorableError(err) {
			...
			rh.requestLogger.LogRequest(c, originalGroup, group, apiKey, startTime, 499, err, isStream, upstreamURL, channelHandler, bodyBytes, nil, models.RequestTypeFinal, modelAlias)
			return nil
		}
		...
		rh.requestLogger.LogRequest(c, originalGroup, group, apiKey, startTime, statusCode, errors.New(parsedError), isStream, upstreamURL, channelHandler, bodyBytes, nil, requestType, modelAlias)
		...
		if isLastAttempt && originalGroup.GroupType == "aggregate" {
			if rh.trySwitchToAnotherSubGroup(c, originalGroup, group, modelAlias, finalBodyBytes, isStream, startTime, excludedSubGroupIDs, statusCode, errorMessage) {
				return nil
			}
		...
		return rh.ExecuteRequestWithRetry(c, channelHandler, originalGroup, group, finalBodyBytes, isStream, startTime, retryCount+1, modelAlias, excludedSubGroupIDs)
	}
...
func (rh *RetryHandler) trySwitchToAnotherSubGroup(
	...
	newSelection, err := rh.subGroupManager.SelectSubGroupExcluding(originalGroup, modelAlias, excludedSubGroupIDs)
	...
	result := rh.ExecuteRequestWithRetry(c, newChannelHandler, originalGroup, newGroup, updatedBodyBytes, isStream, startTime, 0, modelAlias, excludedSubGroupIDs)
	return result != nil
}
